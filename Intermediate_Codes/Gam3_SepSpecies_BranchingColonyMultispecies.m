...
%     end 
    ...
    ga1 = 22;
    ga2 = 20;
    gammas = [gi+ga1*initialFract(1)+ga2*initialFract(3), ...
        gi+ga1*initialFract(1)+ga2*initialFract(3), ...
        2*gi+ga1*initialFract(1)+ga2*initialFract(3)];
...
%     end
    ...
    for i = 0 : nt
        ...
        if mod(i, 0.2/dt) == 0 && initialFract(j) > 0
            ...
            % gamma = gammas_i(j) + gamma_a1*mean(C{1}, 'all') + gamma_a2*mean(C{3}, 'all');
            % dl = gamma * dE(1:nn,j) ./ Width;
            ...
            % relocate dBiomass
            %BEGIN proportioning bit
            Rmax = max(sqrt(Tipx(1, :) .^ 2 + Tipy(1, :) .^ 2));
            r1 = -(Rmax + 9.6)*initialFract(3) + (Rmax + 4.4);
            r2 = (Rmax + 9.6)*initialFract(1) - 4.4;
            calcHY = @(rad) 1/(1+exp(-rad + r1));
            calcCH = @(rad) 1/(1+exp(rad - r2));
            if j == 1 %wildtype 
                fracCapJ = 1 - arrayfun(calcCH, rr) - arrayfun(calcHY, rr);
            elseif j == 2 %cheater
                fracCapJ = arrayfun(calcCH, rr);
            elseif j == 3 %hyperswarmer
                fracCapJ = arrayfun(calcHY, rr);
            end 
            %END proportioning bit
            Capacity = Cmax - C_pre{1} - C_pre{2} - C_pre{3};    % remaining cell capacity
            Capacity = Capacity .* fracCapJ; % APS frac capacity species J can use
    ...
    figname = "SepSpeciesv1_" + strjoin(string(initialRatio), "");
    %figname = descriptive(taskID) + "_" + strjoin(string(initialRatio), "");
